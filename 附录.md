...
plt.rcParams['font.sans-serif'] = ['SimHei', 'Microsoft YaHei', 'Arial', 'Helvetica']
plt.rcParams['axes.unicode_minus'] = False
...
# 绘制多条曲线
ax.plot(x, y1, label='sin(x)', linewidth=1.5, color='#0072B2')
ax.plot(x, y2, label='cos(x)', linewidth=1.5, color='#D55E00', linestyle='--')
ax.plot(x, y3, label='sin(x)·exp(-0.1x)', linewidth=1.5, color='#009E73', linestyle=':')

# 添加复杂注释
# 注释1：正弦波峰值
peak1_x = np.pi / 2
peak1_y = np.sin(peak1_x)
ax.annotate('峰值点1',
            xy=(peak1_x, peak1_y),
            xytext=(peak1_x + 1.5, peak1_y + 0.3),
...
# 注释2：余弦波零点
zero2_x = np.pi / 2
zero2_y = np.cos(zero2_x)
ax.annotate('零点',
            xy=(zero2_x, zero2_y),
            xytext=(zero2_x - 2, zero2_y - 0.5),
...
# 注释3：衰减趋势
ax.annotate('衰减趋势',
            xy=(8, y3[np.argmin(np.abs(x - 8))]),
            xytext=(6, -0.8),
            arrowprops=dict(arrowstyle='->', color='#009E73', linewidth=0.8, 
...
ax.set_title('复杂函数对比分析', fontsize=10, fontweight='bold')

# 自定义图例位置
ax.legend(loc='upper right', frameon=True, fancybox=False, shadow=False, 
          bbox_to_anchor=(1.0, 0.95), borderaxespad=0)
...
# 手动保存多格式
os.makedirs('output', exist_ok=True)
fig.savefig('output/complex_annotation.png', dpi=300, facecolor='white', 
            bbox_inches='tight')
fig.savefig('output/complex_annotation.pdf', format='pdf', bbox_inches='tight')
fig.savefig('output/complex_annotation.svg', format='svg', bbox_inches='tight')
fig.savefig('output/complex_annotation.eps', format='eps', bbox_inches='tight')
...
with PlotTool('ieee') as tool:
    ...
    # 绘制曲线（样式自动应用IEEE规范）
    ax.plot(x, y1, label='sin(x)', color='#0072B2')
    ax.plot(x, y2, label='cos(x)', color='#D55E00', linestyle='--')
    ax.plot(x, y3, label='sin(x)·exp(-0.1x)', color='#009E73', linestyle=':')
    
    # 添加复杂注释（与Matplotlib语法完全兼容）
    peak1_x = np.pi / 2
    peak1_y = np.sin(peak1_x)
    ax.annotate('峰值点1',
                xy=(peak1_x, peak1_y),
    ...
    # 设置标签（字号自动应用IEEE规范）
    tool.set_labels('时间 (s)', '振幅', '复杂函数对比分析')
    
    # 自定义图例
    ax.legend(loc='upper right', bbox_to_anchor=(1.0, 0.95), borderaxespad=0)
    ...
    # 一键保存多格式
    tool.save('output/complex_annotation', bbox_inches='tight')
```
C.5 配置文件驱动示例
